...
loss = tf.reduce_mean(tf.nn.l2_loss((y_ - y)))
...
train_step2 = tf.train.AdagradOptimizer(0.01).minimize(loss)
...
sess = tf.Session()
sess.run(init)
summary_writer = tf.train.SummaryWriter('summary/loss', graph=sess.graph)
